...
export function HistoryStrip({
  ...
  if (!shown.length) {
    return (
      <p className="text-xs text-muted">
        還沒有紀錄。計算過的股票會快取在這裡，點一下立刻打開上次結果，不必重抓財報。
      </p>
    );
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="text-xs text-subtle">快取</span>
      {shown.map((row) => {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-bg/70">
      ...
      <aside className="flex h-full w-full max-w-md flex-col border-l border-line bg-surface">
        <div className="flex items-center justify-between border-b border-line px-4 py-3">
          <div>
            <p className="text-xs tracking-wide text-muted">CACHE</p>
            <h2 className="font-display text-xl">計算快取</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="px-4 pt-3 text-xs text-muted">
          同一檔只留最近一次。「打開」用快取，不用等網路；「再算」才重抓財報與新聞。
        </p>
